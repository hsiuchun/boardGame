...
public class Sdg extends JFrame implements ActionListener,Runnable{
    ...
    public Sdg() {
        ...
        flipJPanel();
        // add(panel);
        // c = (CardLayout) (panel.getLayout());
        // c.show(panel, "1");
        // for(int i=0;i<20;i++){
        //     panel.add(content(i),String.format("%d",i+1));
        // }
        // add(prev);
        // add(next);
        // add(go);
    ...
    public void flipJPanel(){
        for(int i=0;i<14;i++){
            // pages[i]=content(i);
            // panel.add(pages[i],String.format("%d",i+1));
